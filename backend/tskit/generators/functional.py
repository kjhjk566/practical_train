...
def random_walk(
    ...
    values = np.cumsum(np.random.normal(
        mean, std, size=len(index)), dtype=dtype)
...
def sine(
    ...
    if period is None:
        period = len(index)
    values = np.asarray(amplitude * np.sin(2 * np.pi *
                        np.arange(len(index)) / period + phase), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def cosine(
    ...
    if period is None:
        period = len(index)
    values = np.asarray(amplitude * np.cos(2 * np.pi *
                        np.arange(len(index)) / period + phase), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def gaussian(
    ...
    index = generate_index(start=start, end=end, length=length, freq=freq)
    values = np.asarray(np.random.normal(
        mean, std, size=len(index)), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def uniform(
    ...
    values = np.asarray(np.random.uniform(
        low, high, size=len(index)), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def step(
    ...
    index = generate_index(start=start, end=end, length=length, freq=freq)
    values = np.asarray(
        delta * np.floor(np.arange(len(index)) / step_size), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def exponential(
    ...
    index = generate_index(start=start, end=end, length=length, freq=freq)
    values = np.asarray(np.random.exponential(
        scale, size=len(index)), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def log_normal(
    ...
    index = generate_index(start=start, end=end, length=length, freq=freq)
    values = np.asarray(np.random.lognormal(
        mean, std, size=len(index)), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def gamma(
    ...
    index = generate_index(start=start, end=end, length=length, freq=freq)
    values = np.asarray(np.random.gamma(
        shape, scale, size=len(index)), dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
...
def square(
    ...
    index = generate_index(start=start, end=end, length=length, freq=freq)
    values = np.asarray(amplitude * np.arange(len(index)) /
                        period * 2 % 2 < 1, dtype=dtype)
    return tskit.TimeSeries(index=index, values=values, name=name)
