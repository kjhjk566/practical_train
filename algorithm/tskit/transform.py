...
def to_shapelet(
    ...
    for i in range(len(shapelet)):
        shapelet[i] -= (shapelet[i] - shapelet[0]) * \
            (i / (len(shapelet) - 1)) ** alpha
    if inplace:
        return ts
    return tskit.TimeSeries(
        index=ts.index.copy(),
...
def tile(
    ...
    """
    ts_ = tskit.transform.interpolate(
        ts, method=interpolate_method, fill_value=fill_value, period_length=period_length)
    index = tskit.generators.generate_index(
        start=ts_.index[0], length=len(ts_.index) * n, freq=ts_.freq)
...
def connect(
        ts1: tskit.TimeSeries,
        ts2: tskit.TimeSeries,
        interpolate_method: str = 'linear',
        fill_value: float = 0.0,
        period_length: int | pd.offsets.BaseOffset | None = None,
        name: str | None = None,
    ...
    """
    Connect two time series

    Parameters
    ----------
    ts1: tskit.TimeSeries
        The time series to connect.
    ts2: tskit.TimeSeries
        The time series to connect.
    interpolate_method: str, optional, default: 'linear'
        The interpolation method to use. See `tskit.transform.interpolate` for more details.
    ...
    period_length: int or pd.offsets.BaseOffset, optional, default: None
        The history period to use for interpolation. This is only used when `method` is set to 'history'.
    name: str, optional, default: None
        The name of the connected time series. The default is the name of the original TimeSeries with '_'.

    Returns
    -------
    tskit.TimeSeries
        The connected time series.
    """
    ts1_ = tskit.transform.interpolate(
        ts1, method=interpolate_method, fill_value=fill_value, period_length=period_length)
    ts2_ = tskit.transform.interpolate(
        ts2, method=interpolate_method, fill_value=fill_value, period_length=period_length)
    index = tskit.generators.generate_index(
        start=ts1_.index[0], length=len(ts1_.index)+len(ts2_), freq=ts1_.freq)
    values = np.append(ts1.values, ts2.values)
    return tskit.TimeSeries(index=index, values=values, name=f'{ts1.name}_{ts2.name}' if name is None else name)
...
def resize(
    ...
    ----------
    ts: tskit.TimeSeries
        The time series to resize.

    length:int 
        The excepted goal length of the time series 

    name: str, optional, default: None
    The name of the connected time series. The default is the name of the original TimeSeries with '_resized'.
    Returns
    -------
    tskit.TimeSeries
        The resized time series.
    """
    data = ts.values
    old_index = np.arange(len(ts))
    new_index = np.linspace(0, len(ts)-1, length)
    f = interp1d(old_index, data, kind='linear')
    index = tskit.generators.generate_index(
        start=ts.index[0], length=length, freq=ts.freq)
    new_data = f(new_index)
    return tskit.TimeSeries(index=index, values=new_data, name=f'{ts.name}_resized' if name is None else name)
...
def interpolate(
    ...
    new_index = tskit.generators.generate_index(
        start=ts.index[0], end=ts.index[-1], freq=ts.freq)
