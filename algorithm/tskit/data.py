...
class TimeSeries:
    ...
    def from_generators(
        ...
        generator_objs = [tskit.utils.deserialize(
            g, obj_type='generator') for g in generators]
        ...
        if len(generator_args) != len(generator_objs):
            raise ValueError(
                'The number of generator arguments must match the number of generators.')
        series = [g(**args, **kwargs)()
                  for g, kwargs in zip(generator_objs, generator_args)]
        series = tskit.transform.add(
            series, weights=weights, standardize_idx=standardize_idx)
        return series
    ...
    def from_dataframe(
        ...
        if to_datetime:
            index = pd.DatetimeIndex(pd.to_datetime(
                df[index_col], unit=timestamp_unit))
        else:
    ...
    def plot(
        ...
        """
            ...
            **kwargs
            ...
            """
        tskit.plot(self, ax=ax, tight_layout=tight_layout,
                   show=show, figsize=figsize, title=title, **kwargs)
