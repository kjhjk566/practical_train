...
def triangle(length, freq=0.04, coef=1.5, offset=0.0, noise_amp=0.05):
    timestamp = np.arange(length)
    period_num = int(length * freq)  # 4
    period_len = length // period_num  # 25
    if period_len < 1:
        period_len = 1
    temp_list = []
    for i in range(length):
        if int(i * freq * 2) % 2 == 0:
            temp_list.append(i - (i // period_len) * period_len)
        else:
            temp_list.append((i // period_len+1) * period_len - i)
    # value = np.sin(2 * np.pi * freq * timestamp)
    value = np.array(temp_list)
    if noise_amp != 0:
...
def square_triangle(level=5, length=500, freq=0.04, coef=1.5, offset=0.0, noise_amp=0.05):
    value = np.zeros(length)
    for i in range(level):
        value += 1 / (2 * i + 1) * triangle(length=length, freq=freq * (2 * i + 1), coef=coef, offset=offset, noise_amp=noise_amp)
    return value
...
class MultivariateDataGenerator:
    ...
    def collective_global_outliers(self,dim_no, ratio, factor, radius, option='square', coef=3., noise_amp=0.0,
        ...
        valid_option = {'square', 'triangle', 'other'}
        ...
        if option == 'square':
            ...
            sub_data = square_triangle(level=level, length=self.STREAM_LENGTH, freq=freq,
                                   coef=coef, offset=offset, noise_amp=noise_amp)
            # sub_data = collective_global_synthetic(length=self.STREAM_LENGTH, base=base,
            #                                        coef=coef, noise_amp=noise_amp)
        for i in position:
            start, end = int(max(0, i - radius)), int(min(self.STREAM_LENGTH, i + radius))
            self.data[dim_no][start:end] = (sub_data[start:end] * factor + self.data[dim_no][start:end]) / (1 + factor)
    ...
    def collective_trend_outliers(self,dim_no, ratio, factor, radius):
        ...
        # position = (np.random.rand(round(self.STREAM_LENGTH * ratio / (2 * radius))) * self.STREAM_LENGTH).astype(int)
        T = 300
        position = (np.random.rand(round(self.STREAM_LENGTH * ratio / (2 * radius + T))) * self.STREAM_LENGTH).astype(int)
        for i in position:
            # start, end = max(0, i - radius), min(self.STREAM_LENGTH, i + radius)
            # slope = np.random.uniform(-1, 1) * factor
    ...
    def split(self, a, n):
        k, m = divmod(len(a), n)
        n= int(n)
        return [a[int(i * k + min(i, m)):int((i + 1) * k + min(i + 1, m))] for i in list(range(n))]
    ...
    def collective_seasonal_outliers(self,dim_no, ratio, factor, radius):
        ...
        for i in position:
            start, end = int(max(0, i - radius)), int(min(self.STREAM_LENGTH, i + radius))
            length = end - start
            tmp =  self.data[dim_no][start:end]
            tmp = self.split(tmp, length//factor+1)
            result = []
            for i in range(int(factor)):
                result.extend([rd.choice(i) for i in tmp])

            indices = rd.sample(range(len(result)), len(result)-length)
            indices.sort(reverse=True)  
            for index in indices:
                result.pop(index)
            self.data[dim_no][start:end] = result
            self.label[start:end] = 1

if __name__ == '__main__':
    # np.random.seed(10)
    # print()
    # plt.plot(triangle(100))
    # plt.ylabel('值')
    # plt.xlabel('索引')
    # plt.title('折线图')
    # plt.show()
    ...
    df.to_csv(r"C:\projects\python\tods-benchmark\benchmark\synthetic\testdataset\01234.csv", index=False)
    ...
    # plt.savefig(r"C:\projects\python\tods-benchmark\benchmark\synthetic\testdataset\01234.jpg")
    # plt.show()
